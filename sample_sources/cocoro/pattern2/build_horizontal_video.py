...
IMAGES = BASE / "images"
AUDIO = BASE / "audio_aivis_mai_normal"
...
Style: JA,Yu Gothic UI Semibold,50,&H00FFFFFF,&H000000FF,&H66453325,&H99453325,1,0,0,0,100,100,0,0,1,4,1,2,150,150,76,1
...
def video_filter(ass: Path | None = None) -> str:
    filters = [
...
def make_segment(scene_id: str, image: Path, audio: Path, ja: str) -> Path:
    ...
    run([
        ...
        "-vf", video_filter(ass), "-c:v", "libx264", "-pix_fmt", "yuv420p", "-r", "30",
...
def main():
    ...
    if ENDING_HOLD_SECONDS and rows:
        ...
        run([
            "ffmpeg", "-y", "-loop", "1", "-t", str(ENDING_HOLD_SECONDS), "-i", str(rows[-1][1]),
            "-f", "lavfi", "-t", str(ENDING_HOLD_SECONDS), "-i", "anullsrc=channel_layout=stereo:sample_rate=44100",
            "-vf", video_filter(), "-c:v", "libx264", "-pix_fmt", "yuv420p", "-r", "30",
    ...
    rough = OUTPUT / "cocoro_pattern2_horizontal_mai.mp4"
    final = OUTPUT / "cocoro_pattern2_horizontal_mai_clean.mp4"
    run(["ffmpeg", "-y", "-f", "concat", "-safe", "0", "-i", str(list_file), "-c", "copy", "-movflags", "+faststart", str(rough)])
    run(["ffmpeg", "-y", "-i", str(rough), "-c:v", "libx264", "-pix_fmt", "yuv420p", "-r", "30", "-c:a", "aac", "-b:a", "192k", "-movflags", "+faststart", str(final)])
    print(final)
